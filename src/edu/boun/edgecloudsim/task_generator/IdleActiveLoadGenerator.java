...
public class IdleActiveLoadGenerator extends LoadGeneratorModel{
	...
	public void initializeModel() {
		...
		AbstractRealDistribution[][] rngList = new AbstractRealDistribution[SimSettings.APP_TYPES.values().length][3];
		...
		for(int i=0; i<SimSettings.APP_TYPES.values().length; i++) {
			...
			rngList[i][0] = new ExponentialDistribution(rng, SimSettings.getInstance().getTaskLookUpTable()[i][5]);
			rng = new Well19937c(SimSettings.getInstance().getRandomSeed());
			rngList[i][1] = new ExponentialDistribution(rng, SimSettings.getInstance().getTaskLookUpTable()[i][6]);
			rng = new Well19937c(SimSettings.getInstance().getRandomSeed());
			rngList[i][2] = new ExponentialDistribution(rng, SimSettings.getInstance().getTaskLookUpTable()[i][7]);
		}
		...
		for(int i=0; i<numberOfMobileDevices; i++) {
			...
			if (SimSettings.getInstance().getTaskLookUpTable()[randomTaskType.ordinal()][12] < 0) {
				//// implement distribution as uniform instead of exponential
				Well19937c rng = new Well19937c(SimSettings.getInstance().getRandomSeed());
				UniformRealDistribution rngDistribution = new UniformRealDistribution(rng, 0, 1); // needs changing **PROBABLY WRONG
				while(virtualTime < simulationTime) {
					double interval = 1;
					//double interval = rngDistribution.sample(); // needs changing
					...
					boolean wifi = true;
					EdgeTask newTask = new EdgeTask(i, randomTaskType, virtualTime, rngList/**/, wifi, sens, act);
					newTask.setLength((long) SimSettings.getInstance().getTaskLookUpTable()[randomTaskType.ordinal()][7]);
					newTask.setInputFileSize((long) SimSettings.getInstance().getTaskLookUpTable()[randomTaskType.ordinal()][5]);
					newTask.setOutputFileSize(1);
							//EdgeTask newTask = new EdgeTask(i, randomTaskType, virtualTime, rngList/**/, wifi, sens, act);
			...
			else { //// added by Bobby (RVS) to allow for different task distributions
