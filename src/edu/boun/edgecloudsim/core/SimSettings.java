...
public class SimSettings {
	...
	public static enum APP_TYPES { AUGMENTED_REALITY, HEALTH_APP, HEAVY_COMP_APP, INFOTAINMENT_APP, COGNITIVE_ASSISTANCE, REMOTE_HEALTHCARE, MACHINE_LEARNING, IMAGE_RECOGNITION }
    ...
    private double[][] taskLookUpTable = new double[APP_TYPES.values().length][13]; // changed from 12 to 13
	...
	private void parseApplicationsXML(String filePath)
	{
		...
		try {	
			...
			for (int i = 0; i < appList.getLength(); i++) {
			    ...
			    //taskLookUpTable[appType.ordinal()][12] = appName;
			    if (appName.equals("IMAGE_RECOGNITION")) {
			    	taskLookUpTable[appType.ordinal()][12] = -1;
			    }
			    else {
			    	taskLookUpTable[appType.ordinal()][12] = 0;
			    }
			}
